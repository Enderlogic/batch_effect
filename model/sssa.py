...
from model.components.beta import BetaVAE_MLP
from model.components.transforms import NormalizingFlow
from model.metrics.correlation import compute_mcc
from model.metrics.block import compute_r2


class SSA(pl.LightningModule):

    def __init__(self, input_dim, c_dim, s_dim, nclass, n_flow_layers, optimizer="adam", embedding_dim=0,
                 hidden_dim=128, bound=5, count_bins=8, order='linear', lr=1e-4, beta=0.0025, gamma=0.001, sigma=1e-6,
                 sigma_x=0.1, sigma_y=None, vae_slope=0.2, use_warm_start=False, spline_pth=None,
                 decoder_dist='gaussian', correlation='Pearson', encoder_n_layers=3, decoder_n_layers=1, scheduler=None,
                 lr_factor=0.5, lr_patience=10, hz_to_z=True):
        ...
        # Inference
        self.net = BetaVAE_MLP(input_dim=self.input_dim + self.embedding_dim, output_dim=self.input_dim,
                               z_dim=self.z_dim, slope=vae_slope, encoder_n_layers=encoder_n_layers,
                               decoder_n_layers=decoder_n_layers, hidden_dim=hidden_dim)

        # Spline flow model to learn the noise distribution
        self.spline_list = []
        for i in range(self.nclass):
            spline = NormalizingFlow(input_dim=s_dim, n_layers=n_flow_layers, bound=bound, count_bins=count_bins,
                                     order=order)

            if use_warm_start:
                spline.load_state_dict(torch.load(spline_pth, map_location=torch.device('cpu')))
    ...
    def validation_step(self, batch, batch_idx):
        x, c, y = batch['x'], batch['domain'], batch['source']
        batch_size, _ = x.shape
        if self.embedding_dim > 0:
            x = torch.cat([x, self.embeddings(c.squeeze().long())], dim=1)
        c = torch.squeeze(c).to(torch.int64)
    ...
    def configure_optimizers(self):
        if self.optimizer.lower() == "adam":
            opt_v = torch.optim.AdamW(filter(lambda p: p.requires_grad, self.parameters()), lr=self.lr,
    ...
    def embed(self, batch):
        self.eval()
        x, c = torch.tensor(batch['x'], dtype=torch.float32), torch.tensor(batch['domain'], dtype=torch.float32)
        c = torch.squeeze(c).to(torch.int64)
        if self.embedding_dim > 0:
            x = torch.cat([x, self.embeddings(c.squeeze().long())], dim=1)
        _, _, _, zs = self.net(x)
        ...
        mask = one_hot.reshape(-1, self.nclass)
        es = (es * mask.unsqueeze(-1)).sum(1)
        es = es.reshape(x.shape[0], self.s_dim)
        return torch.cat((zs[:, :self.c_dim], es), dim=1).detach().numpy()
